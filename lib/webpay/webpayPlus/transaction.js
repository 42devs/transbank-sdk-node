...
exports.create = async (payload, options = null) => {
  const opts = createOptions(options);
  requests.create.validate(payload);
  if (!requests.create.isValid()) throw Error(requests.create.validationErrors());
  try {
...
exports.commit = async (token, options = null) => {
  const opts = createOptions(options);
  try {
    const response = await request(
      `${endpoint}/${token}`,
...
exports.status = async (token, options = null) => {
  const opts = createOptions(options);
  try {
    const response = await request(
      `${endpoint}/${token}`,
...
exports.capture = async (token, payload, options = null) => {
  ...
  try {
    ...
    if (!requests.capture.isValid()) requests.capture.validationErrors();
